...
def assign_celltypes(save_path, n_clusters, cell_types_dict, thresholding_dict, thresholded_matrix_path, phenotype_clusters_path, cluster_arr_path, channel_names, output_suffix = 'cell_types'):
    ...
    cluster_arr = np.load(cluster_arr_path)
    cell_types_df, mixed_cell_stats = assign_mixed_celltype(cell_types_df, cluster_arr, thresholding_dict, channel_names)
    ...
    with open(os.path.join(output_path, 'mixed_cell_stats.json'), 'w') as json_file:
        json.dump(mixed_cell_stats, json_file, indent=4)
...
def assign_mixed_celltype(cell_types_df, cluster_arr, thresholding_dict, channel_names):

    print("Cell types df shape: ", cell_types_df.shape)
    print("Cell cluster array shape: ", cluster_arr.shape)
    print('CD3e channel index:', channel_names.index('CD3e'))

    assert cell_types_df.shape[0] == cluster_arr.shape[0], "Number of cells in cell types df and matrix do not match"
    ...
    for sample in np.unique(cell_types_df['slide_id'].values):
        ...
        sample_cluster_arr = cluster_arr[sample_mixed_cells.index - 1]
        print(f"Sample cluster array shape: {sample_cluster_arr.shape}")

        assert sample_mixed_cells.shape[0] == sample_cluster_arr.shape[0], "Number of cells in heterogenous cell types df and matrix do not match"
