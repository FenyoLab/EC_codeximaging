...
def pca_by_sample_indiv(pca_path, sample_ids_path, plot_dir):
    # Load the data
    ...
    for sample_i, sample_name in enumerate(unique_samples):
        ...
        plt.scatter(pca_embedding[:, 0], pca_embedding[:, 1], s=0.1, c='lightgray', label='all')
        plt.scatter(pca_embedding[sample_indices, 0], pca_embedding[sample_indices, 1], s=0.1, c=sample_color, label=sample_name)
        ...
        plot_file_path = f'{plot_dir}/pca_{sample_name}.png'
