...
def omero_label_image(base_dir, seg_data_path, image_id_dict, channel_names, tile_size = 256):
    '''Function to upload label images from a csv input to OMERO, also creates a marker intensity omero table
    Inputs: base_dir - str, path to the research drive
    ...
    research_drive_path = os.path.join(f'/mnt/{kerberosid}', base_dir)
    ...
    for sample in np.unique(cell_sample_names):
        ...
        sample_research_drive_path = os.path.join(research_drive_path, sample)
        ...
        server_directory = os.path.join('/omero', base_dir, sample)
